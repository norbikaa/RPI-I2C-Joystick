...
void updateUInputDevice(int UInputFIle, I2CJoystickStatus *newStatus, I2CJoystickStatus *status) {
  ...
  TestBitAndSendKeyEvent(status->buttons, newStatus->buttons, 4, KEY_UP);
  TestBitAndSendKeyEvent(status->buttons, newStatus->buttons, 5, KEY_DOWN);
  TestBitAndSendKeyEvent(status->buttons, newStatus->buttons, 6, KEY_LEFT);
  TestBitAndSendKeyEvent(status->buttons, newStatus->buttons, 7, KEY_RIGHT);
